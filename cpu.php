<?php
/* ---------------------------------------------------- */
/* ç¨‹åºåç§°: PHPæŽ¢é’ˆ-Yahei
/* ç¨‹åºåŠŸèƒ½: æŽ¢æµ‹ç³»ç»Ÿçš„WebæœåŠ¡å™¨è¿è¡ŒçŽ¯å¢ƒ
/* ç¨‹åºå¼€å‘: Yahei.Net
...
/* ---------------------------------------------------- */
/* æ„Ÿè°¢ä»¥ä¸‹æœ‹å‹ä¸ºæŽ¢é’ˆåšå‡ºçš„è´¡çŒ®:
/* zyypp,é…·ã‚’é¾™å·é£Ž,é¾™æ™ºè¶…,èŠèŠ±è‚¿äº†,é—²äºº,Clare Lou,hotsnow
/* äºŒæˆ’,yexinzhu,wangyu1314,Kokgog,gibyasus,akw28888,Aå¤§,huli
/* å°æ¾,charwin,åŽæ™¯ç½‘ç»œ
/* æ‚¨å¯èƒ½æ˜¯ä¸‹ä¸€ä¸ª?
/* ---------------------------------------------------- */
...
{
	$speed=round(100/($_GET['speed']/1000),2); //ä¸‹è½½é€Ÿåº¦: $speed kb/s
}
else
{
...
function sys_linux()
{
    ...
    $res['memRealUsed'] = $res['memTotal'] - $res['memFree'] - $res['memCached'] - $res['memBuffers']; //çœŸå®žå†…å­˜ä½¿ç”¨
	$res['memRealFree'] = $res['memTotal'] - $res['memRealUsed']; //çœŸå®žç©ºé—²
    $res['memRealPercent'] = (floatval($res['memTotal'])!=0)?round($res['memRealUsed']/$res['memTotal']*100,2):0; //çœŸå®žå†…å­˜ä½¿ç”¨çŽ‡

	$res['memCachedPercent'] = (floatval($res['memCached'])!=0)?round($res['memCached']/$res['memTotal']*100,2):0; //Cachedå†…å­˜ä½¿ç”¨çŽ‡
...
function sys_windows()
{
	...
	$res['win_n'] = $sysinfo[0]['Caption']." ".$sysinfo[0]['CSDVersion']." åºåˆ—å·:{$sysinfo[0]['SerialNumber']} äºŽ".date('Yå¹´mæœˆdæ—¥H:i:s',strtotime(substr($sysinfo[0]['InstallDate'],0,14)))."å®‰è£…";
	//UPTIME
...
//åˆ¤æ–­å†…å­˜å¦‚æžœå°äºŽ1Gï¼Œå°±æ˜¾ç¤ºMï¼Œå¦åˆ™æ˜¾ç¤ºGå•ä½
if($sysInfo['memTotal']<1024)
{
	...
	$memRealUsed = $sysInfo['memRealUsed']." M"; //çœŸå®žå†…å­˜ä½¿ç”¨
	$memRealFree = $sysInfo['memRealFree']." M"; //çœŸå®žå†…å­˜ç©ºé—²
	$memRealPercent = $sysInfo['memRealPercent']; //çœŸå®žå†…å­˜ä½¿ç”¨æ¯”çŽ‡
	$memPercent = $sysInfo['memPercent']; //å†…å­˜æ€»ä½¿ç”¨çŽ‡
	$memCachedPercent = $sysInfo['memCachedPercent']; //cacheå†…å­˜ä½¿ç”¨çŽ‡
}
else
{
	...
	$swapPercent = $sysInfo['swapPercent'];
	$memRealUsed = round($sysInfo['memRealUsed']/1024,3)." G"; //çœŸå®žå†…å­˜ä½¿ç”¨
	$memRealFree = round($sysInfo['memRealFree']/1024,3)." G"; //çœŸå®žå†…å­˜ç©ºé—²
	$memRealPercent = $sysInfo['memRealPercent']; //çœŸå®žå†…å­˜ä½¿ç”¨æ¯”çŽ‡
	$memPercent = $sysInfo['memPercent']; //å†…å­˜æ€»ä½¿ç”¨çŽ‡
	$memCachedPercent = $sysInfo['memCachedPercent']; //cacheå†…å­˜ä½¿ç”¨çŽ‡
}
...
<table width="100%" cellpadding="3" cellspacing="0" align="center">
  ...
  <tr>
    <td>Space usage</td>
    <td colspan="5">
		Total Space <?php echo $dt;?>&nbsp;Gï¼Œ
		Used <font color='#333333'><span id="useSpace"><?php echo $du;?></span></font>&nbsp;Gï¼Œ
		Free <font color='#333333'><span id="freeSpace"><?php echo $df;?></span></font>&nbsp;Gï¼Œ
...
//åˆ¤æ–­å¦‚æžœSWAPåŒºä¸º0ï¼Œä¸æ˜¾ç¤º
...
<table>
  ...
  <tr>
   ...
   <tr>
    <td>High-precision math(BCMath)ï¼š</td>
...
<table>
  ...
  <tr>
    <td>SQLite: </td>
    <td><?php if(extension_loaded('sqlite3')) {$sqliteVer = SQLite3::version();echo '<font color=green>âˆš</font>ã€€';echo "SQLite3ã€€Ver ";echo $sqliteVer[versionString];}else {echo isfun("sqlite_close");if(isfun("sqlite_close") == '<font color="green">âˆš</font>') {echo "&nbsp;ï¼š ".@sqlite_libversion();}}?></td>
    <td>Hyperwave: </td>
  ...
  <tr>
    <td>FilePro databaseï¼š</td>
    <td><?php echo isfun("filepro_fieldcount");?></td>
    <td>SyBase databaseï¼š</td>
